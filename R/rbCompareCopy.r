...
prismDir1 <- 'D:/ecology/Climate/PRISM/Acquired 2020/an81' # RNJ
prismDir2 <- 'G:/ecology/Climate/PRISM/Subscription/ORIGINALS' # GRN

rbCompareCopy <- function(
	prismDir1,
	prismDir2,
	dates,
	period = 'monthly',
	...
	for (thisVar in vars) {
		...
		if (period == 'monthly') {
			...
			### get versions from rasters
			infoPath1 <- paste0(prismDir1, '/monthly/', thisVar, yr, '/data_prism_', thisVar, '_us_30s_', yrmo, '/', 'prism_', thisVar, '_us_30s_', yrmo, '.info.info.txt')
			infoPath1 <- paste0(prismDir2, '/monthly/', thisVar, yr, '/data_prism_', thisVar, '_us_30s_', yrmo, '/', 'prism_', thisVar, '_us_30s_', yrmo, '.info.info.txt')
			
			info1 <- read.csv(infoPath1)
			info2 <- read.csv(infoPath2)
